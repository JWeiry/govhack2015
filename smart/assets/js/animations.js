...
'desctext':'I, ANDREW BROWN MCKENZIE, Of 20 \
Harold Street, South Melbourne, in the State \
of Victoria, Comnmonwealth of Australia, \
Shipwright, hereby declare this invention, and \
the manner in which it is to be performed, to \
be fully described and ascertained in and by \
the following statement:- \
A number of serious accidents have occurred \
in connection with the use of the Westinghouse \
brake, and from long experience in the \
railway service I have ascertained one cause \
of such accidents and have devised the present \
means of overcoming it with extreme simplicity \
and economy. \
At each end of each railway carriage and at \
each end of the locomotive there is a lever \
to turn on and off a cock which allows (or \
prevents) the passage of air. This lever is \
at all times in an extraordinarily exposed, unprotected \
and dangerous position considering \
how easily it might be operated, and how \
great is the amount of life and property dependent \
upon its retaining its correct position.',
	'sound':'brake.ogg'
},
...
'patentno':'<a href="http://www.scienceimage.csiro.au/video/12480/wlan-the-next-generation/" target="_blank" class="btn btn-xs outline">VIDEO</a> 1993051806',
'image':'wifi.png',
...
var image= 'Placeholder';
  ...
  function setLens() {
		$(".theimg").mlens(
			{
//				imgSrc: 'assets/img/'+image,	    // path of the hi-res version of the image
...
//	$('body').attr('style','-webkit-filter: hue-rotate('+ Math.floor((Math.random() * 360) + 1) + 'deg);');
$(".theimg").toggleClass('shine2');

		pagenumtest = getUrlVars()["pagenum"];
		console.log(pagenumtest)
		if (pagenumtest > 0)
			{ pagenum = parseInt(pagenumtest); }
		console.log(pagenum);
		if (pagenum == (data.length - 1)) {$('.next').slideUp(0);}
		if (pagenum == 0) {$('.prev').slideUp(0);}
  	  	desctext = data[""+pagenum].desctext;
  	  	title = data[""+pagenum].title;
  	  	image = data[""+pagenum].image;
  	  	patno = data[""+pagenum].patentno;
		
		patnotest = getUrlVars()["patentno"];
		switch(patnotest){
			case '1904000001': window.location.href="?t=1&pagenum=0"; break;
			case '1985039903': window.location.href="?t=1&pagenum=1"; break;
			case '1993051806': window.location.href="?t=1&pagenum=3"; break;
			case '1982085668': window.location.href="?t=1&pagenum=4"; break;
			case '1956016938': window.location.href="?t=1&pagenum=5"; break;
			case '1926004843': window.location.href="?t=1&pagenum=6"; break;
			case '1978041061': window.location.href="?t=1&pagenum=7"; break;
			case '1983015178': window.location.href="?t=1&pagenum=8"; break;
			case '1947015008': window.location.href="?t=1&pagenum=9"; break;
			case '1992023666': window.location.href="?t=1&pagenum=10"; break;
			case '1971030650': window.location.href="?t=1&pagenum=11"; break;
			case '1914015133': window.location.href="?t=1&pagenum=12"; break;
			
		}

		$('.title').html(title);
		$('.patno').html(patno);
		$('.theimg').attr('src','assets/img/' + image)
//		$('.theimg').attr('data-big','assets/img/' + image)
	...
	//		setLens();
		...
		$(".blocker2").animate({
			...
			$(".blocker2").animate({
				...
				// Animation complete.
				$(".blocker2").slideUp(700)
				setInterval(function() {	$(".theimg").toggleClass('shine'); }, 8300);
		...
		if (typeof data[""+pagenum].sound != 'undefined')
			{ $('.desc').prepend('<audio controls> <source src="assets/sound/'+ data[""+pagenum].sound +'" type="audio/ogg"></audio>'); }

		setInterval(function() {
			$('.thetext').append(toappend);
			toappend = desctext.substring(0, 6);
			$('.bright').remove();
			$('.thetext').append('<span class="bright">' + toappend + '</span>');
			desctext = desctext.substring(6);
		}, 90);
		...
		$('.next').on('click', function() {
				if (pagenum > 0) {
					oldnum = '&pagenum=' + pagenum.toString();
					newnum = '&pagenum=' + (parseInt(pagenum) + 1);
					window.location.href = window.location.href.replace(oldnum, newnum);
				} else {
					window.location.href = window.location.href + '?t=1&pagenum=1'
				}
			}
		);
